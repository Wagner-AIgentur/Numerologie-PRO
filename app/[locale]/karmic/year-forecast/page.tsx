...
const t = {
  de: {
    ...
    yearError: 'Bitte gib ein gültiges Jahr ein.',
    allFieldsError: 'Bitte fülle alle Pflichtfelder aus.',
    formula: 'Formel: Alle Ziffern von Tag + Monat + Prognosejahr werden addiert. Ist die Summe > 22, wird 22 abgezogen. Beispiel: 25.12 + 2026 → 2+5+1+2+2+0+2+6 = 20 → Arkana 20.',
    formulaLabel: 'Formel:',
    arcanaPrefix: '→ Arkana',
  },
  ru: {
    ...
    yearLabel: 'Год прогноза *',
    yearPlaceholder: 'напр. 2026',
    nameLabel: 'Имя *',
    ...
    yearError: 'Пожалуйста, введите корректный год.',
    allFieldsError: 'Пожалуйста, заполните все обязательные поля.',
    formula: 'Формула: Все цифры дня + месяца + года прогноза складываются. Если сумма > 22, вычитается 22. Пример: 25.12 + 2026 → 2+5+1+2+2+0+2+6 = 20 → Аркан 20.',
    formulaLabel: 'Формула:',
    arcanaPrefix: '→ Аркан',
  },
};

function calcArcana(birthdate: string, year: number): number | null {
...
export default function YearForecastTestPage() {
  ...
  const isValidYear = !isNaN(forecastYear) && forecastYear >= 1900 && forecastYear <= 2100;
  const isFormValid = isValidBirthdate && isValidYear && name.trim().length > 0;
  ...
  async function handleDownload() {
    ...
    try {
      const res = await fetch('/api/karmic/year-forecast', {
        method: 'POST',
      ...
      a.href = url;
      a.download = `year-forecast-${forecastYear}.pdf`;
      a.click();
  ...
  const arcanaNumber = isValidYear ? calcArcana(birthdate, forecastYear) : null;
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-20">
      ...
      <div className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm">
        ...
        <div>
          <label className="mb-1 block text-sm text-white/70">{l.yearLabel}</label>
          <input
            type="number"
            value={yearStr}
